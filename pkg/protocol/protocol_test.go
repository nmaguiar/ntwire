...
func TestWireJSONFieldsAreDistinct(t *testing.T) {
	b, err := json.Marshal(AuthResponse{SessionID: "id", Token: "token", TunnelIP: "100.64.0.2", ServerPublicKey: "server", Tunnels: []Tunnel{{Name: "db", Description: "database"}}})
	if err != nil {
		t.Fatal(err)
	}
	for _, field := range []string{"\"token\"", "\"tunnel_ip\"", "\"server_public_key\"", "\"description\""} {
