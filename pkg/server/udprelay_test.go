...
func TestUDPRelaySessionForEchoesClientObservedStats(t *testing.T) {
	...
	first := u.sessionFor(context.Background(), peer.Public, false, false, nil)
	if first.Stats != nil {
	...
	second := u.sessionFor(context.Background(), peer.Public, false, false, clientStats)
	if second.Stats == nil {
	...
	third := u.sessionFor(context.Background(), peer.Public, false, false, nil)
...
func TestUDPRelaySessionForIsIdempotent(t *testing.T) {
	...
	resp1 := u.sessionFor(context.Background(), peer.Public, false, false, nil)
	if resp1.Token != "tok-1" || resp1.RelayAddr != "127.0.0.1:1" {
		t.Fatalf("sessionFor() first call = %+v, want token/relay_addr populated", resp1)
	}
	resp2 := u.sessionFor(context.Background(), peer.Public, false, false, nil)
	if resp2 != resp1 {
...
func TestUDPRelaySessionForReturnsEmptyWhenAllocationUnavailable(t *testing.T) {
	...
	resp := u.sessionFor(context.Background(), peer.Public, false, false, nil)
