...
3. (Optional) Check authorized tunnels before connecting:
   ```sh
   ntwire list https://server.example:8443
   ```
   This authenticates once and displays your allowed tunnels, virtual/local ports, and descriptions without opening listeners (`--json` outputs machine-readable data).
4. Connect:
   ```sh
...
5. Check tunnels, transport status, and live traffic:
   ```sh
   ntwire status
   ```
   or open the token-protected status URL `connect` prints. Use `ntwire status --json` for scripts, `ntwire port name=local-port` to rebind ports live, or `ntwire disconnect` to stop the session.

> **Tip:** Generate shell completion for your environment (`bash`, `zsh`, `fish`, `powershell`):
> `source <(ntwire completion zsh)` or `ntwire completion bash > /etc/bash_completion.d/ntwire`
...
2. Write or generate the client profile on the device (iOS, macOS, Windows,
   Android, or Linux official app):

   > **Fastest method:** Generate the ready-to-scan QR code or `.conf` directly from `ntwire-server`:
   > ```sh
   > # Display QR code in terminal for mobile app scanning:
   > ntwire-server -config ntwire.yaml -print-wireguard-qr -wireguard-peer iphone
   > 
   > # Or export .conf file:
   > ntwire-server -config ntwire.yaml -print-wireguard-conf -wireguard-peer iphone > client.conf
   > 
