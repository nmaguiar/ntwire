...
2. Write or generate the client profile on the device (iOS, macOS, Windows,
   ...
   > **Tip:** You can print this configuration or a scannable QR code directly
   > from `ntwire-server` using:
   > `ntwire-server -config ntwire.yaml -print-wireguard-config` (or `-print-wireguard-qr` / `-print-wireguard-conf`).
