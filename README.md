...
| Session renewal, rate limits, reaping, persistent configuration, a status UI, and a settings page for editing that persisted configuration | Settings changes apply on the next `ntwire connect`, not the running connection. |
...
| `ntwire connect [-i key \| --sso] URL [--port name=15432] [--websocket] [--bind address]` | Starts local listeners, renews its session, and prints a token-protected status URL; the status UI's dashboard links to a companion settings page (also token-protected, on its own local port) for editing `~/.ntwire/config.yaml` -- changes there apply on the *next* `ntwire connect`, not this running one. A tunnel's YAML `local_port`/`local_host` are preferred when available, falling back to a free port and to `127.0.0.1` respectively; `--port name=local-port` or `--port name=host:local-port` (e.g. `db=127.70.0.1:5432`) is a client-side override, strict for the port and soft for the host. Same key/SSO selection as `list`; `--websocket` selects fallback transport. `--bind` binds tunnel listeners to an address other than `127.0.0.1` (advanced; see SECURITY.md). |
